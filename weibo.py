...
import gzip, time, json, hmac, base64, hashlib, urllib, urllib.request, logging, mimetypes, collections
...
def _parse_json(s):
    ...
    def _obj_hook(pairs):
        ...
        o = JsonDict()
        # 修改 by duanzhiying：python3不支持iteritems以迭代器对象返回键值对，应该使用items以列表方式返回
...
def _http_call(the_url, method, authorization, **kw):
    ...
    # 修改 by duanzhiying: data.encode('encoding') 将str转为byte，否则resp = urllib.request.urlopen(req, timeout=5)这里会崩溃
    # 参考:http://stackoverflow.com/questions/5440485/typeerror-post-data-should-be-bytes-or-an-iterable-of-bytes-it-cannot-be-str
    # http_body = None if method==_HTTP_GET else params
    http_body = None if method==_HTTP_GET else params.encode(encoding='UTF8')
    ...
    try:
        ...
        # 修改 by duanzhiying: 这里的json解析参数是string，所以需要将byte类型的body转为string。bytes解码会得到str
...
class APIClient(object):
    ...
    # 获取用户的账号信息（主要得到uid）
    def get_uses_show(self,access_token=None):
        access_token = access_token if access_token else self.access_token
        if not access_token:
            raise APIError('01','缺少参数：access_token','获取用户账号信息')
        r = _http_get('%s%s' % (self.api_url, 'users/show.json', ), \
                     access_token=access_token)
        return r

    # 获取用户发表的微博内容
    def get_user_time_line(self,max_id=0,access_token=None):
        access_token = access_token if access_token else self.access_token
        if not access_token:
            raise APIError('01','缺少参数：access_token','获取用户微博')
        r = _http_get('%s%s' % (self.api_url, 'statuses/user_timeline.json', ), \
                     access_token=access_token, \
                     max_id = max_id)
        return r
